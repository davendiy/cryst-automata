...
# This file was *autogenerated* from the file ../working/normalizer.sage
from sage.all_cmdline import *   # import sage library

_sage_const_3 = Integer(3); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2)#!/usr/bin/env sage
...
def create_symbolic_matrix(dim, use_alphabet=False):
    ...
    for i in range(dim):
        ...
        for j in range(dim):
            ...
            args.append(row[-_sage_const_1 ])
        A.append(row)
    A = matrix(A)
    return A, args


def sol2matrix(solution, dim=_sage_const_2 , use_alphabet=False):
...
# noinspection PyShadowingNames
def normalizers_old(n, dim=_sage_const_2 , use_alphabet=False, verbose=False, normalize_exp=True,
    ...
    for perm in permutations(range(len(s_elements))):
        ...
        for i in range(len(s_elements)):
            if S.AsList()[i + _sage_const_1 ].Order() != S.AsList()[perm[i] + _sage_const_1 ].Order():
                check = False
                break
        if not check:
            continue
        ...
        eq = [cond == _sage_const_0  for cond in cond_perm]
        for res in solve(eq, *args):
            ...
            if res not in found_solutions and verbose:
                if to_matrix:
                    ...
                    if ignore_trivial and mtx.det() == _sage_const_0 :
                        continue
                    print(mtx, end='\n\n')
                else:
                    print(res)
...
def _carthesian_wo_duplicates(*spaces):
    ...
    for res, used in _carthesian_wo_duplicates(*spaces[:-_sage_const_1 ]):
        for el in spaces[-_sage_const_1 ]:
            if str(el) in used:
                continue
...
def get_point_group_gens(cryst_num, dim=_sage_const_3 , to_l_basis=False):
    if to_l_basis:
...
#   refactor maybe
# noinspection PyShadowingNames
def normalizers(n, dim=_sage_const_2 , verbose=False, use_alphabet=False,
    ...
    for maps_to in carthesian_wo_duplicates(*possible_mappings):
        ...
        for res in solve(eq, *args):
            ...
            if mtx.det() != _sage_const_0  or not ignore_trivial:
                found_solutions.add(res)
...
def check_div(A):
    ...
    # check rows
    for row in test1:
        if list(row).count(_sage_const_0 ) >= n - _sage_const_1 :
            return True
